...
namespace msocket
{
   ...
   {
   public:
      virtual void tcp_accept(msocket_server_t* server, msocket_t* accepted_socket);
      virtual void udp_msg_received(const std::string& address, std::uint16_t port, const std::uint8_t* data, std::size_t data_size);
